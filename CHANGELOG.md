...
### Added
- **`--disable-read-reassignment` / `--enable-read-reassignment`** (core) — Gates Phase 4b (post-phasing concordance-based read reassignment within identity groups). Default enabled. Disable to preserve original phasing-time membership
- **`--disable-discard-recovery` / `--enable-discard-recovery`** (core) — Gates Phase 4b2 (recovery of previously discarded reads into surviving clusters). Default enabled. Coupled to read reassignment: if `--disable-read-reassignment` is set, discard recovery is automatically skipped (with an INFO log) since the recovery path uses the same concordance machinery. Phase 4b3 (second phasing pass) now runs only when `--enable-position-phasing` AND `--enable-read-reassignment` are both effective — if 4b is off, nothing has changed since the original Phase 3 phasing and 4b3 has no useful work
