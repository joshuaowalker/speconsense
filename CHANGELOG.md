...
- **The new group pruning filter is on by default.** To disable it, pass `--prune-group-ratio 0` or `--prune-group-abs 0`. Existing outputs will change: small secondary groups that previously passed will now route to `.lq`.
