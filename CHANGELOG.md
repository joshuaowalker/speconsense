...
### Added
- **`--disable-read-reassignment` / `--enable-read-reassignment`** (core) — Gates the post-phasing concordance-based read reassignment within identity groups. Default enabled. Disable to preserve original phasing-time membership
- **`--disable-discard-recovery` / `--enable-discard-recovery`** (core) — Gates the recovery of previously-discarded reads into surviving clusters. Default enabled. Coupled to read reassignment: if `--disable-read-reassignment` is set, discard recovery is automatically skipped (with an INFO log) since the recovery path uses the same concordance machinery. The second phasing pass now runs only when `--enable-position-phasing` AND `--enable-read-reassignment` are both effective — if reassignment is off, nothing has changed since the original phasing pass and there is no useful work to do
...
- **`--enable-early-filter` / `--disable-early-filter` removed** (core) — The early filter applied `--min-size` and `--min-cluster-ratio` after pre-phasing merge to skip variant phasing on doomed-small clusters. In practice the flag was disabled by default, no bundled profile enabled it, and the unconditional hard-floor filter (drop clusters < 3 reads, added on the dev branch) covers the most useful case. The post-phasing scalability work (parallel noise-filter SPOA, sparse identity grouping, sparse discard screening, within-group top-K) further weakens the perf argument. Filtering by `--min-size` / `--min-cluster-ratio` continues to happen at the post-phasing size-filtering phase. User profiles still setting `enable-early-filter:` will fail validation — drop the line
