...
sequencing by base and run length, using three independent sequencing runs of
...
(rate ratios of 1.03 [95\% CI: 0.88--1.19], 0.88 [0.79--0.99], and 0.72
[0.61--0.84] across the three runs). These results support extending
...
base, using real amplicon data from three independent sequencing runs spanning
...
We analyzed specimens from three sequencing runs of fungal ITS amplicons
(${\sim}700$~bp), all sequenced on Oxford Nanopore R10.4.1 E8.2 chemistry.
Reads were demultiplexed with specimux and clustered with speconsense using
...
\item \textbf{run116}: 127 qualifying specimens from 794 total (herbarium type
  specimens). Basecalled with Dorado SUP v5.0.0, sequenced December 2025.
\end{itemize}

For ont98 and run116, qualifying specimens had a primary cluster with $\geq
200$ reads representing $\geq 70\%$ of total reads. For ont37, thresholds were
relaxed to $\geq 100$ reads and $\geq 50\%$ dominance due to lower overall
sequencing depth (median c1 read count of 59 vs 200+ for the newer runs).
...
\item \textbf{Our error rates are substantially better than Sereika et
  ...
  (Section~2.6), whereas Sereika measured raw read-level accuracy against a
  known reference.
\end{enumerate}
...
\section{Cross-Run Validation (run116)}

\subsection{Dataset}

To validate the ont98 results, we repeated the analysis on run116, a sequencing
run of fungal type specimens performed approximately 3 months after ont98 using
the same chemistry (R10.4.1 E8.2, Dorado SUP v5.0.0). Run116 contains 794
specimens with c1 MSA files; 127 met the qualifying criteria (primary cluster
$\geq 200$ reads, $\geq 70\%$ dominance). The qualifying rate is lower than
ont98 (16\% vs 29\%), primarily because many type specimens yielded few reads
due to degraded DNA from old herbarium material.

\subsection{Raw error rates}

Non-HP error rates in run116 are slightly higher than ont98 (sub 0.82\% vs
0.59\%, total 1.13\% vs 0.99\%), likely reflecting more biological diversity in
the type collection rather than higher sequencing error.

HP error rates at lengths 1--5 are broadly consistent:

\begin{table}[h]
\centering
\begin{tabular}{rrr}
\toprule
HP Length & ont98 per-pos & run116 per-pos \\
\midrule
1 & 0.72\% & 0.91\% \\
2 & 0.87\% & 0.64\% \\
3 & 1.04\% & 0.76\% \\
...
\subsection{Biological variant filtering}

The raw error rates include HP positions where the observed variation is
biological (genuine length polymorphisms) rather than sequencing error. We
identify these using two detectors:
...
\begin{tabular}{lrrr}
\toprule
Dataset & Outlier positions & Bimodal positions & Total excluded \\
\midrule
ont98 & 1,656 & 106 & 1,657 (1.3\%) \\
run116 & 1,136 & 287 & 1,136 (3.5\%) \\
\bottomrule
\end{tabular}
\end{table}

Run116 has proportionally more biological variants to filter, consistent with
greater taxonomic diversity in the type collection. Notably, run116 has 216
bimodal L=1 positions (vs 10 in ont98), reflecting multi-copy ITS variation.

\subsection{Filtered error rates and cross-run correspondence}

After removing biological variants, the filtered per-position error rates are:

\begin{table}[h]
\centering
\caption{Filtered per-position HP error rates and cross-run scaling (ont98 vs run116).}
\label{tab:filtered}
\begin{tabular}{rrrr}
\toprule
HP Length & ont98 filtered & run116 filtered & run116/ont98 \\
\midrule
1 & 0.67\% & 0.39\% & 0.58 \\
2 & 0.83\% & 0.52\% & 0.63 \\
3 & 0.97\% & 0.62\% & 0.64 \\
...
Filtering reveals a pattern that was obscured by biological variant noise:
run116 error rates at L=1--5 are uniformly ${\sim}$60--65\% of ont98's. This
consistent scaling factor suggests a systematic improvement in sequencing
quality between runs---plausible given the ${\sim}$3~month gap and the
sensitivity of nanopore sequencing to flow cell quality, library preparation,
and basecaller calibration. The biological variants, being specimen-dependent
rather than technology-dependent, introduce noise that obscures this underlying
pattern.

At L~$\geq 6$, the ratio breaks down due to small sample sizes and the
increasing influence of individual outlier positions.
...
(vs v5.0.0 for ont98 and run116). The data was combined from two sample IDs
(ONT037 and ONT37B), with 70\% of reads basecalled using the standard SUP
v3.5.1 model and 30\% using the 260bps SUP v3.5.2 model. All three datasets
use R10.4.1 E8.2 chemistry.
...
observations: 10.2~million (vs 82.0~million for ont98 and 75.6~million for
run116).
...
\begin{table}[h]
\centering
\caption{Non-HP error rates across all three datasets.}
\label{tab:nonhp_all}
\begin{tabular}{lrrrr}
\toprule
...
run116 (v5.0) & 0.82\% & 0.31\% & 0.54\% & 1.67\% \\
\bottomrule
\end{tabular}
\end{table}

The ont37 non-HP total error rate of 3.97\% is 2.4$\times$ higher than the
v5.0 datasets. All error classes are elevated: substitutions 2.4$\times$,
deletions 2.5$\times$, and insertions 2.3$\times$.
...
rates across all three datasets are:

\begin{table}[h]
\centering
\caption{Filtered per-position HP error rates: three-run comparison.}
\label{tab:three_run}
\begin{tabular}{rrrrrr}
\toprule
HP Len & ont37 & ont98 & run116 & ont37/ont98 & ont98/run116 \\
\midrule
1 & 1.56\% & 0.67\% & 0.39\% & 2.32 & 1.72 \\
2 & 1.84\% & 0.83\% & 0.52\% & 2.21 & 1.60 \\
3 & 2.17\% & 0.97\% & 0.62\% & 2.23 & 1.56 \\
4 & 2.27\% & 0.99\% & 0.64\% & 2.30 & 1.54 \\
5 & 2.13\% & 1.13\% & 0.70\% & 1.89 & 1.62 \\
6 & 2.90\% & 1.57\% & 2.06\% & 1.85 & 0.76 \\
7 & 4.21\% & 2.23\% & 1.77\% & 1.89 & 1.26 \\
\bottomrule
\end{tabular}
\end{table}

At L=1--5, ont37 error rates are consistently ${\sim}$2.2$\times$ those of
ont98. Combined with the ${\sim}$0.6$\times$ scaling from ont98 to run116, the
full range from oldest to newest basecaller spans a factor of ${\sim}$3.5--4.0.
This demonstrates substantial improvement from Dorado v3.5 to v5.0, a larger
gap than the run-to-run variation observed between ont98 and run116 (which
share the same basecaller).
...
shorter lengths compared to the v5.0 datasets (max/min ratio of 1.54 at L=1
...
\subsection{Interface variant rate (ont37)}

The expanded interface analysis on ont37 (651 specimens evaluated, with cluster
recombination):

\begin{table}[h]
\centering
\caption{Interface vs non-HP variant rate ratio across all three datasets.}
...
run116 & 304 & 0.721 & 0.611--0.842 & $2.1 \times 10^{-7}$ \\
\bottomrule
\end{tabular}
\end{table}

All three datasets are consistent: interface HP positions do not show excess
...
(speconsense 0.7.7, 2026-04-21) on the same ont98 and run116 input data to
validate or update the shipped values.
...
Both ont98 (955 specimens) and run116 (959 specimens) were re-processed with
the current pipeline defaults, preserving only the sampling configuration
that matches the original analysis:
...
\texttt{-{}-hp-min-length~6}, \texttt{-{}-min-variant-count~3},
...
pulling the primary fraction below 0.70), and on run116 it drops to
\emph{zero}. We therefore switched to an \texttt{err\_factor}-based criterion:
the primary anchor cluster must have \texttt{err\_factor}~$<~1.0$ and
\texttt{RiC}~$\geq~200$. This yields 445 qualifying specimens on ont98 and 148
on run116. The criterion is peer-independent (does not depend on sibling
structure), directly measures single-template plausibility, and is stable
...
and only one additional run116 specimen would be admitted by relaxing
the gate (149 vs.\ 148). \texttt{RiC} is therefore the effective
selection criterion; \texttt{err\_factor} serves here as a backup
guarantee rather than an active filter. This distinction matters for
the circularity argument: because $q_\text{ctx}$ enters the selection
only through \texttt{err\_factor}, and \texttt{err\_factor} is not
constraining our pool, the rate estimates are not circularly dependent
on the $q_\text{ctx}$ value used to compute them. Section~\ref{sec:revised_qctx}
confirms this analytically under the proposed table revision.
...
\begin{table}[h]
\centering
\caption{Filtered per-position error rates: original analysis vs.\
...
\begin{tabular}{lrrrr}
\toprule
 & \multicolumn{2}{c}{ont98} & \multicolumn{2}{c}{run116} \\
\cmidrule(lr){2-3} \cmidrule(lr){4-5}
Context & original & v2\_ef & original & v2\_ef \\
\midrule
non-HP sub          & 0.0059 & 0.0039 & 0.0082 & 0.0023 \\
non-HP del          & 0.0040 & 0.0028 & 0.0031 & 0.0012 \\
non-HP ins          & 0.0068 & 0.0046 & 0.0054 & 0.0022 \\
\midrule
hp-l1               & 0.0067 & 0.0047 & 0.0039 & 0.0028 \\
hp-l2               & 0.0083 & 0.0062 & 0.0052 & 0.0036 \\
hp-l3               & 0.0097 & 0.0076 & 0.0062 & 0.0046 \\
hp-l4               & 0.0099 & 0.0081 & 0.0064 & 0.0043 \\
hp-l5               & 0.0113 & 0.0087 & 0.0070 & 0.0055 \\
\bottomrule
\end{tabular}
\end{table}

The ratio between ont98 and run116 is preserved ($\sim$0.6$\times$), matching
the cross-run scaling already documented in the CER-in-practice
paper~\cite{cer_practice}~\S5.4 and Table~\ref{tab:filtered} of this
analysis. This is a sequencing-run-level effect, not a pipeline artifact.
...
On run116, rates decline monotonically with \texttt{RiC} (from 0.0037 at 5--20
to 0.0016 at 1000+). This reflects specimen quality heterogeneity in the
herbarium-type material rather than cluster-size-dependent pipeline behavior:
large clusters in run116 are drawn from cleaner templates, small clusters
from partially-degraded templates.

\begin{table}[h]
\centering
\caption{All-cluster non-HP error rates by \texttt{RiC} bin. Each row pools
reads across all MSAs (\texttt{RiC}~$\geq~5$) in the specimen, using per-
cluster consensus as the ground-truth scaffold.}
\label{tab:approach2}
\small
\begin{tabular}{lrrrrrr}
\toprule
 & \multicolumn{3}{c}{ont98} & \multicolumn{3}{c}{run116} \\
\cmidrule(lr){2-4} \cmidrule(lr){5-7}
Bin (\texttt{RiC}) & Clusters & Sub & Sub+Del & Clusters & Sub & Sub+Del \\
\midrule
5--20             & 5,472 & 0.0046 & 0.0078 & 18,248 & 0.0037 & 0.0063 \\
20--50              & 632 & 0.0036 & 0.0058 & 1,767 & 0.0031 & 0.0048 \\
50--100             & 324 & 0.0039 & 0.0063 &   719 & 0.0030 & 0.0047 \\
100--200            & 289 & 0.0037 & 0.0062 &   456 & 0.0031 & 0.0047 \\
200--500            & 408 & 0.0039 & 0.0066 &   386 & 0.0030 & 0.0044 \\
500--1000           & 203 & 0.0038 & 0.0066 &   256 & 0.0024 & 0.0035 \\
1000--$\infty$        & 29 & 0.0037 & 0.0065 &    67 & 0.0016 & 0.0024 \\
\midrule
POOLED           & 7,357 & 0.0039 & 0.0066 & 21,899 & 0.0028 & 0.0044 \\
\bottomrule
\end{tabular}
\end{table}
...
\texttt{-{}-hp-min-length~0}, which reverts to the old behavior of blanket-
...
\begin{table}[h]
\centering
\caption{Effect of disabling HP phasing (\texttt{-{}-hp-min-length~0}) on
...
\item \textbf{iNat/herbarium input distribution.} The measurements reflect
specimen quality distributions typical of iNaturalist uploads (ont98) and
herbarium-type material (run116). Environmental DNA with substantially
higher input-quality variance could produce different all-cluster rates.
\item \textbf{Run-to-run variation (${\sim}0.6\times$) remains the dominant
uncertainty.} The shipped table is a reasonable centroid for ont98-like
runs; run116-like conditions give lower rates. The CER factor's linear
sensitivity to $q_\text{ctx}$~\cite{cer_practice}~\S3 handles this by
design.
...
The combined results from three independent sequencing runs spanning two
basecaller generations support a simple two-tier approach:

\textbf{Short homopolymers (length 1--5):} Implied per-position error rates
are consistently below 2.3\% (filtered) across all three datasets, all bases,
and both basecaller versions. For the current Dorado v5.0 basecaller, rates
are below 1.4\%. These positions can be brought into the CER framework---no
...
per-run estimation to account for flow-cell variation (Section~8.3) is left as
future work.
...
variants, confirmed across all three datasets (rate ratio 1.03 in ont37, 0.88
in ont98, 0.72 in run116).
...
\item Well-supported variants at HP interface positions occur at or below the
  ...
  0.88 [95\% CI: 0.79--0.99]; run116 ratio 0.72 [95\% CI: 0.61--0.84]). There
  is no excess that would indicate a signal-level artifact, even with the
  noisier v3.5 basecaller.
...
All three datasets show substantially better HP accuracy than published R10.4
...
(Section~2.6). The quality filtering inherent in clustering excludes the
...
rates (Section~2.6), not directly comparable to Sereika's raw read-level
measurements.}
\begin{tabular}{rrrrr}
\toprule
HP Len & Sereika R10.4 & ont37 (v3.5) & ont98 (v5.0) & run116 (v5.0) \\
\midrule
1 & ${\sim}$99.2\% & 98.4\% & 99.3\% & 99.6\% \\
3 & ${\sim}$94.5\% & 93.6\% & 97.1\% & 98.2\% \\
5 & ${\sim}$86.0\% & 89.8\% & 94.5\% & 96.6\% \\
7 & ${\sim}$60.0\% & 74.0\% & 85.4\% & 88.2\% \\
\bottomrule
\end{tabular}
\end{table}

The ont37 data (Dorado v3.5) provides an intermediate point between the
published Guppy results and our v5.0 data, demonstrating a clear trajectory of
improvement. Even at the v3.5 level, short HP accuracy is sufficient for CER
analysis. The three-run comparison reveals two distinct sources of improvement:
a large basecaller-version effect (${\sim}$2.2$\times$ from v3.5 to v5.0) and
a smaller run-to-run effect (${\sim}$0.6$\times$ from ont98 to run116 within
v5.0). Both argue for per-run error rate estimation rather than fixed
parameters.
...
\item \textbf{Pipeline-conditioned measurement}: Our error rates are effective
  ...
  threshold and clustering parameters (Section~2.6). They are not directly
...
\item \textbf{Run-specific error rates}: The ${\sim}$2.2$\times$ scaling factor
  between basecaller versions (v3.5 to v5.0) and ${\sim}$0.6$\times$ factor
  between runs on the same basecaller confirm that HP error rates should be
  estimated per-run rather than assumed universal. Both basecaller upgrades and
  run-to-run variation (flow cell quality, library preparation) contribute to
  differences.
...
\item \textbf{Biological variant filtering}: The outlier/bimodal detection
  ...
  sufficient to reveal the cross-run scaling pattern.
\end{enumerate}
