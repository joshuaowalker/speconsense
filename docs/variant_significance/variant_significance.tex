...
joint probability under the null that all $K$ positions simultaneously exhibit a
variant-like pattern drops exponentially. Although the combinatorial correction
$\binom{L}{K}$ grows with $K$, the $K$-th root dominates, and any multi-position
variant is at least as significant as a single-position variant. As a concrete
example, at $N = 1000$, $M = 100$: $K = 1$ gives $p^* = 6.75\%$, while $K = 2$
gives $p^* = 7.12\%$. The combinatorial correction grows from $L = 700$ to
$\binom{700}{2} = 244{,}650$ (a $350{\times}$ increase), but the squared
per-position probability overwhelmingly compensates. The analysis that follows
therefore uses $K = 1$ throughout as the conservative bound.
...
variants are distinguished at multiple positions, further increasing significance.
\end{enumerate}
